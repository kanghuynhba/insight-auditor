...
"""Extracts section text from a PDF document.

Mirrors ``EpubTextExtractor.extract_texts()``: the single public
classmethod ``extract_texts()`` accepts a ``fitz.Document`` and a
``TocNode`` root, and returns ``Dict[str, str]`` mapping each node's
``href`` (the page-number string set by ``PdfTocBuilder``) to its
extracted Markdown text.

The key insight from the user: in a PDF, section boundaries live at
the *page* level.  Each TOC entry carries the page it starts on; its
text ends where the next entry (in reading order) begins.  This is
the PDF analogue of the EPUB stop-anchor mechanism.

Text extraction strategy
────────────────────────
PyMuPDF's ``page.get_text("dict")`` returns a block/line/span tree
that preserves font sizes and positions.  We rotate through that tree
to reconstruct clean Markdown:

* Heading-sized spans  → ``#`` / ``##`` / ``###`` prefixes
* Normal body spans    → plain paragraph text
* Form feeds and soft  → normalised by ``clean_markdown_text``
  hyphens

Because PDF text is not pre-structured as HTML we skip the
container/sibling logic that EPUB needs and instead rely entirely on
page-range slicing.
"""
...
from src.infrastructure.loaders.common.markdown_converter import clean_markdown_text

logger = logging.getLogger(__name__)

# Font-size → Markdown heading prefix
...
class PdfTextExtractor:
    """Stateless extractor – all methods are classmethods, just like EpubTextExtractor."""

    @classmethod
    def extract_texts(cls, doc: fitz.Document, toc_root: TocNode) -> Dict[str, str]:
        """Extract text for every TOC section and return href → Markdown.

        ``href`` values are the 1-based page numbers encoded as strings
        by ``PdfTocBuilder``.  Sections that produce no usable text are
        omitted from the result (matching EPUB extractor behaviour).
        """
